      ...
      <div className="max-w-5xl mx-auto">
        ...
        <label className="block mb-4">
          ...
          <input
            ...
            className="mt-1 block w-full rounded-xl border px-4 py-3"
            value={token}
        ...
        <div className="mb-4">
          <button
            className="bg-[color:var(--accent)] text-white px-4 py-3 rounded-2xl"
            onClick={load}
            disabled={!token}
          >
            Load RSVPs
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {rsvps.map((r) => (
              <div
                key={r.id}
                className="glass-card p-4 flex flex-col sm:flex-row sm:items-center sm:gap-4 justify-between"
              >
                <div className="flex-1">
                  <div className="font-semibold">
                    {r.name} <span className="text-sm text-slate-500">• {r.email}</span>
                  </div>
                  <div className="text-sm text-slate-600 mt-1">
                    {r.attending} • {r.guests} guests
                  </div>
                ...
                <div className="flex items-center gap-3 mt-3 sm:mt-0">
                  <button
                    ...
                    aria-label={`Set ${r.name} visibility`}
                    className={`px-3 py-2 rounded-full ${r.visibility === 'PUBLIC' ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}
                    onClick={() => toggleVisibility(r.id, r.visibility)}
                  >
